...
# Register product routes directly because the current application
# initialization flow prevents include_router() from retaining them.
_existing_paths = {
    (
        getattr(route, "path", ""),
...
for route in admin_products.router.routes:
    route_key = (
        getattr(route, "path", ""),
        tuple(sorted(getattr(route, "methods", set()) or set())),
    )

    if route_key not in _existing_paths:
        app.router.routes.append(route)
        _existing_paths.add(route_key)
