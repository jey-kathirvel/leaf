...
SESSION_SECRET = os.getenv(
    "SESSION_SECRET_KEY",
    os.getenv("SECRET_KEY", "leaf-store-change-this-session-secret"),
)
...
@app.get("/health")
async def health():
    return {"status": "ok", "application": settings.APP_NAME, "environment": settings.APP_ENV}


@app.get("/manifest.webmanifest", include_in_schema=False)
async def web_manifest():
    return FileResponse(BASE_DIR / "static" / "manifest.webmanifest", media_type="application/manifest+json", headers={"Cache-Control": "public, max-age=3600"})


@app.get("/service-worker.js", include_in_schema=False)
async def service_worker():
    return FileResponse(BASE_DIR / "static" / "service-worker.js", media_type="application/javascript", headers={"Cache-Control": "no-cache, no-store, must-revalidate", "Service-Worker-Allowed": "/"})


@app.get("/offline", include_in_schema=False)
async def offline_page():
    return FileResponse(BASE_DIR / "static" / "offline.html", media_type="text/html", headers={"Cache-Control": "public, max-age=86400"})
...
from app.routers.admin_configuration import router as admin_configuration_router
