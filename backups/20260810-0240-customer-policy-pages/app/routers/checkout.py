...
def context(request: Request, cart=None, **values):
    ...
    return {"request": request, "current_year": datetime.now().year, "cart": cart, "totals": totals, "cart_count": totals["count"], **values}
...
def checkout_submit(
    ...
    if len(full_name.strip()) < 2:
        ...
        error = "Cash on Delivery is currently the available payment method."

    if not error:
        try:
            form["mobile"] = re.sub(r"\D", "", mobile)
            order = place_cod_order(db, cart, form)
...
def order_confirmation(order_number: str, request: Request, db: Session = Depends(get_db)):
    ...
    if not order:
        return RedirectResponse("/", status_code=303)
    return templates.TemplateResponse(request, "store/order_confirmation.html", context(request, None, order=order))
